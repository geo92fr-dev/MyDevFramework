...
/**
 * Script de validation CheckBeforeDoing - Vérifie la c    validateCheckBeforeDoingDocument() {
        this.log('info', 'Validation du document CheckBeforeDoing...');
        
        if (!fs.existsSync('DOC_CoPilot_Practices.md')) {
            this.log('error', 'Document CheckBeforeDoing manquant (DOC_CoPilot_Practices.md)');
            return false;
        }

        const content = fs.readFileSync('DOC_CoPilot_Practices.md', 'utf8'); des prompts
 * Usage: npm run validate:checkbeforedoing
 */

const fs = require('fs');
const path = require('path');

class CheckBeforeDoingValidator {
    constructor() {
        this.errors = [];
        this.warnings = [];
    ...
    generateReport() {
        console.log('\n' + '='.repeat(60));
        console.log('📊 RAPPORT DE VALIDATION CHECKBEFOREDOING');
        ...
        if (this.errors.length === 0 && this.warnings.length === 0) {
            console.log('\n🎉 Projet conforme au CheckBeforeDoing ! Aucun problème détecté.');
        }
    ...
    run() {
        console.log('🤖 Validation CheckBeforeDoing - Check Before Doing');
        console.log(`📅 ${new Date().toLocaleString()}\n`);

        this.validateEnvironment();
        this.validateProjectStructure();
        this.validatePackageJson();
        this.validateCheckBeforeDoingDocument();
...
if (require.main === module) {
    const validator = new CheckBeforeDoingValidator();
    validator.run();
}

module.exports = CheckBeforeDoingValidator;
